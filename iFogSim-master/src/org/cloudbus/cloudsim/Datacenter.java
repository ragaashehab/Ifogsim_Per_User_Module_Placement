...
public class Datacenter extends SimEntity {
	...
	protected void processVmCreate(SimEvent ev, boolean ack) {
		//System.out.println("datacenter processvmcreate : ev " +  ev + ev.getSource()+ev.getDestination()+ ev.getType()+ ev.getData().toString());
		Vm vm = (Vm) ev.getData();
		...
		if (ack) {
			...
			System.out.println("datacenter processvmcreate : ev "+result +  ev +" vm "+ ((AppModule)vm).getName()+ ack);
		...
		if (result) {
			...
			//System.out.println( this.getVmList().size());

			if (vm.isBeingInstantiated()) {
				vm.setBeingInstantiated(false);
				//vm.setBeingInstantiated(true);  // ragaa changed this
			}
	...
	protected void processVmMigrate(SimEvent ev, boolean ack) {
		...
		Datacenter datacenter = (Datacenter) migrate.get("datacenter");

		getVmAllocationPolicy().deallocateHostForVm(vm);
		host.removeMigratingInVm(vm);
		boolean result = getVmAllocationPolicy().allocateHostForVm(vm, host);
		//System.out.println("datacenter processvmmigrate==========================================");
		//System.out.println("==========================================");
		//System.out.println("datacenter processvmmigrate allocatehostfor vm "+vm.getId() +  " hostid "+ host.getId() + "datacenterid "+ datacenter.getId() + datacenter.getName());
		...
		if (ack) {
			...
			//System.out.println("send ack  ev source datacenter"+ ev.getSource()+ "data[0] id= "+ getId() +  this.getName()+
				//	"data[1] vm.getId()"+  vm.getId() + "result " + data[2]);
			sendNow(ev.getSource(), CloudSimTags.VM_CREATE_ACK, data);
			//sendNow(datacenter.getId(), CloudSimTags.VM_CREATE_ACK, data);
		}
...
////////////////////////////////////////////////////////////////////////////////////
	...
	protected void processCloudletMove(int[] receivedData, int type) {
		...
		if (type == CloudSimTags.CLOUDLET_MOVE_ACK) {// send ACK if requested
			...
			System.out.println("datacenter processcloudletmove cl"+ cl);
			System.out.println("datacenter processcloudletmove userid "+ cl.getUserId());
			System.out.println("datacenter processcloudletmove data "+data);
			sendNow(cl.getUserId(), CloudSimTags.CLOUDLET_SUBMIT_ACK, data);
		}
	}
	
	
/////////////////ragaa updates method so send cloudlets to the device contains the migrated modules//////////////////////////////////////////
	/**
	...
	protected void processCloudletSubmit(SimEvent ev, boolean ack) {
		...
		try {
			...
			Cloudlet cl = (Cloudlet) ev.getData();
			//System.out.println("datacenter processcloudletsubmit datacenter "+getName()+ ev);  //// "Cloudlet "+ cl.getCloudletId()	
			...
			if (cl.isFinished()) {
				...
				if (ack) {
					...
					//System.out.println("cl is finishedUserId(), tag, data" + cl.getUserId() + " "+tag + " "+ data);
					sendNow(cl.getUserId(), tag, data);
				}
				
				//System.out.println("cl is finished cl.UserId(), tag, data" + cl.getUserId() + " "+cl);
				sendNow(cl.getUserId(), CloudSimTags.CLOUDLET_RETURN, cl);
...
//////////////////////////	////////////////////////////////////////////
//	System.out.println("datacenter processcloudletsubmit "+ this.getId() +" "+ getName()+ " processing vm "+vmId+" vm "+ ((AppModule)vm).getName()+" ev "+ ev);
////////////////////////////////////////////////////////////////////
			//System.out.println(this.getName()+ " cl doesnot finished UsrId "+ cl.getUserId()+" vmId "+vmId+" vm "+ ((AppModule)vm).getName());
			//System.out.println(" scheduler "+ scheduler+ " estimatedFinishTime:  "+estimatedFinishTime);			
			//System.out.println("Host "+vm.getHost().getId());
			...
			if (estimatedFinishTime > 0.0 && !Double.isInfinite(estimatedFinishTime)) {
				...
				//System.out.println(getName()+ " send to id :"+this.getId()+ this.getName() + " : ESTIMATED FINISH TIME ON "+((AppModule)vm).getName()+": "+estimatedFinishTime);
				send(getId(), CloudSim.getMinTimeBetweenEvents()
					+estimatedFinishTime, CloudSimTags.VM_DATACENTER_EVENT);
			}
			...
			if (ack) {
				...
				// unique tag = operation tag
				int tag = CloudSimTags.CLOUDLET_SUBMIT_ACK;
				System.out.println("cl doesnot finished UserId(), tag, data" + cl.getUserId() + " "+tag + " "+ data);
				sendNow(cl.getUserId(), tag, data);
			}
		} catch (ClassCastException c) {
...
//////////////////////source code//////////////////////////
	...
	protected void processCloudletSubmit(SimEvent ev, boolean ack) {
		...
		try {
			// gets the Cloudlet object
			Cloudlet cl = (Cloudlet) ev.getData();
			System.out.println("datacenter processcloudletsubmit Cloudlet "+ cl.getCloudletId() + " datacenter "+getName());
			// checks whether this Cloudlet has finished or not
			if (cl.isFinished()) {
				String name = CloudSim.getEntityName(cl.getUserId());
				Log.printLine(getName() + ": Warning - Cloudlet #" + cl.getCloudletId() + " owned by " + name
	...
	protected void updateCloudletProcessing() {
		...
		if (CloudSim.clock() < 0.111 || CloudSim.clock() > getLastProcessTime() + CloudSim.getMinTimeBetweenEvents()) {
			...
			if (smallerTime != Double.MAX_VALUE) {
				System.out.println("datacenter updateCloudletProcessing shedule vmdatacenterevent"+ getId());
