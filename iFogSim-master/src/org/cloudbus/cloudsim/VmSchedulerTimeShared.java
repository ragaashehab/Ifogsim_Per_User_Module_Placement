...
public class VmSchedulerTimeShared extends VmScheduler {
	...
	public boolean allocatePesForVm(Vm vm, List<Double> mipsShareRequested) {
		 ...
		 */
		//System.out.println("vmschedtimeshare host "+ vm.getHost()+ " allocatepesforvm VmsMigratingIn "+getVmsMigratingIn()+ "  VmsMigratingOut "+ getVmsMigratingOut());
		//+ "datacenter " +vm.getHost().getDatacenter().getName()
		if (vm.isInMigration()) {
			//System.out.println("vmschedtimeshare vm "+ ((AppModule)vm).getName()+ vm.getHost()+" isinmigration");
			if (!getVmsMigratingIn().contains(vm.getUid()) && !getVmsMigratingOut().contains(vm.getUid())) {
				getVmsMigratingOut().add(vm.getUid());				
				//System.out.println("vmschedtimeshare allocatepesforvm  addto migrating out vm " + ((AppModule)vm).getName()+ "  VmsMigratingOut "+ getVmsMigratingOut());
		...
		//System.out.println("===========================================================");
		//System.out.println("vmschedtimeshare befor allocatepesfor vmid "+vm.getId()+ " vm.getUid "+ vm.getUid()+"  "+((AppModule)vm).getName()+" getMipsMap() "+getMipsMap());//+ "  VmsMigratingIn "+getVmsMigratingIn()+ "  VmsMigratingOut "+ getVmsMigratingOut());
		boolean result = allocatePesForVm(vm.getUid(), mipsShareRequested);
		updatePeProvisioning();

		//System.out.println("vmschedtimeshare after allocatepesforvm "+vm.getId()+ "  "+((AppModule)vm).getName()+" getMipsMap() "+getMipsMap());//+ "  VmsMigratingIn "+getVmsMigratingIn()+ "  VmsMigratingOut "+ getVmsMigratingOut());
		//System.out.println("===========================================================");
		//System.out.println("vmschedtimeshare allocate" +vm.getUid() +" "+ result+"------------------------------------------------------");
		//for(String uid : getMipsMap().keySet()){
		//	System.out.println(uid+"\t--op->\t"+getMipsMap().get(uid));
		//}
	...
	protected boolean allocatePesForVm(String vmUid, List<Double> mipsShareRequested) {	
		//System.out.println("ragaanew allocatePesForVm string");
		...
		//System.out.println("ragaanew totalRequestedMips"+totalRequestedMips+"peMips"+peMips+"getAvailableMips"+getAvailableMips());
		...
		//System.out.println("ragaanew mipsShareAllocated"+mipsShareAllocated+"getMipsMap"+getMipsMap()+"getAvailableMips"+getAvailableMips());
	...
	protected void updatePeProvisioning() {
		//System.out.println("ragaanew updatePeProvisioning ");
		getPeMap().clear();
		for (Pe pe : getPeList()) {
	...
	public void deallocatePesForVm(Vm vm) {
		...
		for (Map.Entry<String, List<Double>> entry : getMipsMapRequested().entrySet()) {
			//System.out.println("vmschedtimeshare deallocatepesforvm "+vm.getId()+ "  "+((AppModule)vm).getName()+ entry.getKey()+ entry.getValue());
