...
import org.fog.scheduler.TupleSchedulerSpaceShared;
...
public class Application {
	...
	public void addAppModule(String moduleName, int ram){
		...
		long bw = 1000;
		String vmm = "Xen";
		//if not added let modprty =1
		AppModule module = new AppModule(FogUtils.generateEntityId(), moduleName, appId, userId, 
				mips, ram, bw, size, vmm, new TupleScheduler(mips, 1), new HashMap<Pair<String, String>, SelectivityModel>(), 1);
...
////////////////////////////////////////////////////////////////////////////////
	//override addappmodule//with module priority ////////////////////////////ragaa
	public void addAppModule(String moduleName,int ram, int mips, long size, long bw, int modprty){ 
		//System.out.println("application addappmodule "+ moduleName +" vm and tuplescheduler mips " + mips);
		String vmm = "Xen";
		AppModule module = new AppModule(FogUtils.generateEntityId(), moduleName, appId, userId, 
				mips, ram, bw, size, vmm, new TupleScheduler(mips, 1), new HashMap<Pair<String, String>, SelectivityModel>(), modprty);		
		// 	TupleScheduler(double mips, int numberOfPes) {
		
		getModules().add(module);
		}
	//////////////////////////////////////////////////////////////////////////////////ragaa

	/////////////////space shared modules
	/**
	 * Adds an application module to the application.
	 * @param moduleName
	 * @param ram
	 */
	public void addAppModuleSpace(String moduleName, int ram){
		int mips = 1000;
		long size = 10000;
		long bw = 1000;
		String vmm = "Xen";
		...
		AppModule module = new AppModule(FogUtils.generateEntityId(), moduleName, appId, userId, 
				mips, ram, bw, size, vmm, new TupleSchedulerSpaceShared(mips, 1), new HashMap<Pair<String, String>, SelectivityModel>(),1);
	...
	//////priority edge//////////////////////ragaa///////////////////////////////////	
	/**
	 * Adds a non-periodic edge to the application model. with user priority
	 * @param source
	 * @param destination
	...
	public void addAppEdge(String source, String destination, double tupleCpuLength, 
			double tupleNwLength, String tupleType, int direction, int edgeType, double uPrty) {
		// TODO Auto-generated method stub
		AppEdge edge = new AppEdge(source, destination, tupleCpuLength, tupleNwLength, tupleType, direction, edgeType, uPrty);
		getEdges().add(edge);
		getEdgeMap().put(edge.getTupleType(), edge);
		//System.out.println("the userpriority is  "+ edge.getUsrPrty());			
		if(!TupleTypeToAverageCpuTimeREQ.containsKey(tupleType)){
		TupleTypeToAverageCpuTimeREQ.put(tupleType, 0.1);
		}
	}
//////////////////////////ragaa//////////////////////////ragaa///////////////////////////////////
	...
	public void addAppEdge(String source, String destination, double tupleCpuLength, 
		...
		//System.out.println("application addappedge "+ tupleType +" tuplecpulength "+ tupleCpuLength);
	...
	public List<AppEdge> getPeriodicEdges(String srcModule){
		...
		//System.out.println("application getPeriodicEdges " + result);
		return result;
	}
				...
				if(selectivityModel.canSelect()){
					...
					if(edge.getEdgeType() == AppEdge.ACTUATOR){
						//for(Integer actuatorId : module.getActuatorSubscriptions().get(edge.getTupleType())){
							...
							//double TPrty=  ragaa 
							//tuple.setTPrty(TPrty);  ragaa
							tuples.add(tuple);
							///////raga							
							//System.out.println("resultanttuples  actualtupid "+ tuple.getActualTupleId()+ "tuple priority"+ tuple.getTPrty()+" cloudletid " + tuple.getCloudletId() + " Srcmod "+ tuple.getSrcModuleName() +" Destmod "+ tuple.getDestModuleName()+ " actuatorid "+ tuple.getActuatorId()+ " cloudletlength "+tuple.getCloudletLength()+ " tupletype "+ tuple.getTupleType());
							/////////raga
						//}
					}else{
						Tuple tuple = new Tuple(appId, FogUtils.generateTupleId(), edge.getDirection(),  
	...
	public Tuple createTuple(AppEdge edge, int sourceDeviceId, int sourceModuleId){
		...
		if(edge.getEdgeType() == AppEdge.ACTUATOR){
			for(Integer actuatorId : module.getActuatorSubscriptions().get(edge.getTupleType())){
				...
				tuple.setSourceModuleId(sourceModuleId);
				/////////raga
				//System.out.println("creattuple actualtupid "+ tuple.getActualTupleId()+ " cloudletid " + tuple.getCloudletId() + " Srcmod "+ tuple.getSrcModuleName() +" Destmod "+ tuple.getDestModuleName()+ " actuatorid "+ tuple.getActuatorId()+ " cloudletlength "+tuple.getCloudletLength()+ " tupletype "+ tuple.getTupleType());
				/////////raga
...
//////////////////////////////////////////////////////////////////////////////////Ragaa
private List<Integer> idOfEndDevices = new ArrayList<Integer>();	
private Map<Integer, Double> deadlineInfo = new HashMap<Integer,Double>();  // mobile id need modulename execute with deadline double
private Map<Integer,Map<String, Double>> deadlineInfomodule = new HashMap<Integer, Map<String,Double>>();  // mobile id need modulename execute with deadline double

private Map<Integer, Double> additionalMipsInfo = new HashMap<Integer, Double>(); // mobile id need additional mips int
private Map<String, Double> TupleTypeToAverageCpuTimeREQ= new HashMap<String, Double>();
private Map<AppEdge, Double> EdgePrty= new HashMap<AppEdge, Double>();;
private Map<Integer, FogDevice> deviceById = new HashMap<Integer,FogDevice>();
private String placement;
private double MobileCPUload;
private Map<Integer, List<Map<String, Double>>> deadlineInfomodule1;
	public void setdeviceById(Map<Integer, FogDevice> deviceById) {
		this.deviceById=deviceById;		
	}
	public Map<Integer, FogDevice> getdeviceById() {
		return deviceById;	
	}
	public void setidOfEndDevices(List<Integer> idOfEndDevices) {
		this.idOfEndDevices=idOfEndDevices;		
	}
	public List<Integer> getidOfEndDevices() {		
		return idOfEndDevices;
	}
	public void setDeadlineInfo(Map<Integer, Double> deadlineInfo) {
		this.deadlineInfo = deadlineInfo;
	}
	public Map<Integer, Double> getDeadlineInfo(){ 
		return deadlineInfo; 
	}
	public void setDeadlineInfomodule(Map<Integer,Map<String, Double>> deadlineInfomodule) {
	this.deadlineInfomodule = deadlineInfomodule;
	}
	public Map<Integer,Map<String, Double>> getDeadlineInfomodule(){ 
	return deadlineInfomodule; 
	}
	...
	public void setAdditionalMipsInfo(Map<Integer, Double> additionalMipsInfo) {
	this.additionalMipsInfo = additionalMipsInfo; 
	}
	public Map<Integer, Double> getAdditionalMipsInfo() { 
	return additionalMipsInfo;
	}
	public void setTupleTypeToAverageCpuTimeREQ(Map<String, Double> TupleTypeToAverageCpuTimeREQ) {  // ragaa new re mapping conditions 
	this.TupleTypeToAverageCpuTimeREQ = TupleTypeToAverageCpuTimeREQ;
	}
	public Map<String, Double> getTupleTypeToAverageCpuTimeREQ(){
	return TupleTypeToAverageCpuTimeREQ;
	}
	public void setEdgePrty(Map<AppEdge, Double> EdgePrty) {
		this.EdgePrty = EdgePrty;
	}
	public Map<AppEdge, Double> getEdgePrty(){ 
		return EdgePrty; 
	}
	public void setMobileCPUload(double mobileCPUload) {
		// TODO Auto-generated method stub
		MobileCPUload= mobileCPUload;
	}
	public double getMobileCPUload() {
		// TODO Auto-generated method stub
		return MobileCPUload;
	}
	public void setDeadlineInfomodule1(Map<Integer, List<Map<String, Double>>> deadlineInfomodule1) {
		// TODO Auto-generated method stub
		this.deadlineInfomodule1= deadlineInfomodule1;
	}

	public Map<Integer, List<Map<String, Double>>> getDeadlineInfomodule1() {
		// TODO Auto-generated method stub
		return deadlineInfomodule1;
	}
//////////////////////////////////////////////////////////////////////////Ragaa	
