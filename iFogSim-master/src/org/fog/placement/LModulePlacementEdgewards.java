...
public class LModulePlacementEdgewards extends ModulePlacement{
	...
	protected double Z;  //ragaanew mobile allowed cpu load

	
	public LModulePlacementEdgewards(List<FogDevice> fogDevices, List<Sensor> sensors, List<Actuator> actuators, 
...
//////////////////////////////////////////////////////////////////////////////////////////
	...
	protected void mapModules() {
		...
		//System.out.println("ragaa device id , module name, module load");
		//System.out.println("ragaa"+ getCurrentModuleLoadMap());
		
		List<List<Integer>> leafToRootPaths = getLeafToRootPaths();
		//System.out.println("leafToRootPaths---------------"+leafToRootPaths);
		
		List<List<Integer>> newList = new ArrayList<List<Integer>>();   
        // Traverse through the first list 
        for (List<Integer> T : leafToRootPaths) { 
            // If this element is not present in newList then add it
            if (!newList.contains(T)) {   
                newList.add(T); 
            } 
        } 	   
        leafToRootPaths=newList;
		//System.out.println("NEW leafToRootPaths---------------"+leafToRootPaths);
...
/////////////////////////////////////////////////////////////////////////////
	...
	private List<String> getModulesToPlace(List<String> placedModules){
		//System.out.println("getModulesToPlace placedModules"+ placedModules);
		...
		//System.out.println("getModulesToPlace modulesToPlace_1"+ modulesToPlace_1);
		/*
		...
		for(String moduleName : modulesToPlace_1){
			...
			for(AppEdge edge : app.getEdges()){
				...
				if(edge.getDestination().equals(moduleName) && edge.getDirection()==Tuple.UP && !placedModules.contains(edge.getSource()))
					toBePlaced = false;
				//System.out.println(moduleName+ "source "+edge.getSource()+ "dest "+ edge.getDestination()+" toBePlaced"+ toBePlaced);
			}
			//System.out.println(moduleName+" toBePlaced"+ toBePlaced);
			if(toBePlaced)
				modulesToPlace.add(moduleName);
			//System.out.println("final modulesToPlace"+ modulesToPlace);
	...
	private void placeModulesInPath(List<Integer> path) {
		...
		for(Integer deviceId : path){
			...
			while(modulesToPlace.size() > 0){ // Loop runs until all modules in modulesToPlace are deployed in the path
				...
				for(AppEdge edge : getApplication().getEdges()){
					//System.out.println("edgeward user:" + path.get(0)+" "+edge.getTupleType()+ " old tuplecpulength "+ edge.getTupleCpuLength());
					edge.setTupleCpuLength(tuplemips);
					//System.out.println("edgeward user:" + path.get(0)+" "+edge.getTupleType()+ " new tuplecpulength "+ edge.getTupleCpuLength());
				}
				...
				if(upsteamDeviceId > 0){
					if(upsteamDeviceId==deviceId){
						...
						//System.out.println("edgeward "+moduleName+ "totalcpuload "+  totalCpuLoad +" currentcpuload" + getCurrentCpuLoad().get(deviceId)+ "on device " + device.getName()+"Z host totalmips "+ Z* device.getHost().getTotalMips());
						if(totalCpuLoad + getCurrentCpuLoad().get(deviceId) > Z*device.getHost().getTotalMips()){
							//System.out.println("edgeward totalcpuload "+  totalCpuLoad +" currentcpuload" + getCurrentCpuLoad().get(deviceId)+ "on device " + device.getName()+" host totalmips "+device.getHost().getTotalMips());
...
/**********************************************************************************************************************
	...
	private List<String> shiftModuleNorth(String moduleName, double cpuLoad, Integer deviceId, List<String> operatorsToPlace) {
		...
		while(true){ // Loop iterates over all devices in path upstream from current device. Tries to place modules (to be shifted northwards) on each of them.
			...
			if(getCurrentCpuLoad().get(id) + totalCpuLoad > Z* fogDevice.getHost().getTotalMips()){
